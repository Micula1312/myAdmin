...
function openRepoEditor(projectId,subId){
  ...
  $('repoTaskList').innerHTML=`<section class="repo-sheet-editor" data-project="${p.id}" data-sub="${s.id}">
    <div class="repo-sheet-grid"><label>Nome repo<input id="sheetRepoName" value="${esc(s.name)}"></label><label>Stato<select id="sheetRepoStatus"><option value="active">Attivo</option><option value="paused">In pausa</option><option value="done">Chiuso</option></select></label><label>GitHub URL<input id="sheetRepoUrl" value="${esc(s.repoUrl||'')}"></label><label>Path locale<input id="sheetRepoPath" value="${esc(s.localPath||'')}" placeholder="Desktop/Movement/Calls/..."></label></div>
    <section class="call-fields">
      <div class="repo-sheet-section-head call-head"><div><p class="eyebrow">CALL / DEADLINE</p><h3>Materiali e scadenza</h3><small>Compila questi campi quando il repo è una call.</small></div></div>
      <div class="call-fields-grid">
        <label>Link call<input id="sheetCallUrl" type="url" value="${esc(s.callUrl||'')}" placeholder="https://..."></label>
        <label>PDF call<input id="sheetCallPdf" value="${esc(s.callPdf||'')}" placeholder="Nome file / path / URL del PDF"></label>
        <label>Deadline<input id="sheetCallDeadline" type="date" value="${esc(s.callDeadline||'')}"></label>
        <label class="calendar-toggle"><span>Calendar</span><span class="calendar-toggle-row"><input id="sheetShowInCalendar" type="checkbox" ${s.showInCalendar?'checked':''}> Mostra la deadline nel calendario</span></label>
      </div>
      <div class="call-links-preview">${s.callUrl?`<a href="${esc(s.callUrl)}" target="_blank" rel="noopener">APRI CALL ↗</a>`:''}${s.callPdf&&/^https?:\/\//i.test(s.callPdf)?`<a href="${esc(s.callPdf)}" target="_blank" rel="noopener">APRI PDF ↗</a>`:''}</div>
    </section>
