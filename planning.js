...
function ensureTodos(){
  ...
  if(!state.todoMigratedV1){
    (state.projects||[]).forEach(p=>(p.subprojects||[]).forEach(s=>(s.tasks||[]).filter(t=>t.status!=='done').forEach(t=>{
      if(!state.todos.some(x=>x.sourceTaskId===t.id))state.todos.push({id:uid('todo'),text:t.text,projectId:p.id,repoId:s.id,status:t.status==='blocked'?'blocked':'open',createdAt:workspaceToday(),completedAt:'',sourceTaskId:t.id});
    })));
...
function renderWorkspaceRepos(){
  ...
  root.innerHTML=rows.length?rows.map(({project:p,sub:s})=>{const todos=todosForRepo(p.id,s.id);return `<article class="workspace-repo-card" data-project="${p.id}" data-repo="${s.id}">
    ...
    <div class="repo-todo-preview">${todos.length?todos.map(t=>`<div class="repo-todo-row ${t.status}"><i></i><span>${esc(t.text)}</span></div>`).join(''):'<span class="empty-inline">Nessuna to do assegnata</span>'}</div>
...
function syncLinkedTask(todo,complete){
  ...
  task.status=complete?'done':task.status;if(complete)task.completedAt=todo.completedAt;
}
$('todoComposer').onsubmit=async e=>{e.preventDefault();const text=$('todoText').value.trim();if(!text)return;let projectId='',repoId='',sourceTaskId='';const v=$('todoRepo').value;if(v)[projectId,repoId]=v.split('|');if(repoId){const r=repoByIds(projectId,repoId);const task={id:uid('t'),text,status:'todo',completedAt:''};r.s.tasks.push(task);sourceTaskId=task.id}state.todos.push({id:uid('todo'),text,projectId,repoId,status:'open',createdAt:workspaceToday(),completedAt:'',sourceTaskId});$('todoText').value='';await saveState(false);workspaceRender()};
